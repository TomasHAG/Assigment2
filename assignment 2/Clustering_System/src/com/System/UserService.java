...
public class UserService { 
	...
	public String ini(@PathParam("nr") String nr) {
		...
		return String.valueOf(sys.KMeans(number));
	}
	
	@GET
	@Path("centroid")
	@Produces(MediaType.APPLICATION_ATOM_XML)
	public String getNumOfCen() {
		return String.valueOf(sys.NumbOfCentroid());
	}
	
	@GET
	@Path("getListCentoid/{nr}")
	@Produces(MediaType.APPLICATION_ATOM_XML)
	public String cenListBlog(@PathParam("nr") String nr) {
		String out = "";
		int in = -1;
		if(areInteger(nr))
			in = Integer.parseInt(nr);
		
		centroid cen = sys.getFromCentroid(in);
		
		for(String blog : cen.listOfBlogs()) {
			out += blog + "\n";
		}
